...
const getTimestampState = (dataset: Record<string, any>) => {
    ...
    const timestamp_key = _.get(dataset_config, "keys_config.timestamp_key");
    return {
        "error": false,
...
const getDataFormatState = (dataset: Record<string, any>) => {
    ...
    const batch_id = extractionConfig.dedup_config.dedup_key
    return {
        "error": false,
        "formFieldSelection": [
            "no",
...
const getDatasetConnectorsConfig = (datasetSourceConfigs: Record<string, any>[]) => {
    const connectorTypes = _.uniq(_.map(datasetSourceConfigs, 'connector_id')) || [];
    return {
        error: false,
        ...
        value: _.reduce(datasetSourceConfigs, (output: Record<string, any>, datasetSourceConfig) => {
            ...
            output[connector_id] = { id, connector_type: connector_id, connector_id, ...connector_config }
            return output;
        }, {}),
    }
}
...
const getDataKeyState = (dataset: Record<string, any>) => {
    ...
    return {
        error: false,
        dataKey: dataset_config?.keys_config?.data_key
    }
}
...
export const fetchDataset = (datasetId: string, status: string) => {
    ...
    const mode = status === 'Draft' ? 'mode=edit' : '';
    const url = `${apiEndpoints.readDataset}/${datasetId}?${mode}&fields=${fields}`;
